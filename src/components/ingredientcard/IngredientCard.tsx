...
export default function IngredientCard({
  ...
  const hasCountries =
    Array.isArray(ingredient.country) && ingredient.country.length > 0;
  const hasCuisines =
    Array.isArray(ingredient.cuisine) && ingredient.cuisine.length > 0;
  const hasFlavors =
    Array.isArray(ingredient.flavorProfile) &&
    ingredient.flavorProfile.length > 0;
  const hasImage = !!ingredient.image?.url;
  const hasComment =
    typeof ingredient.comment === "string" &&
    ingredient.comment.trim().length > 0;
