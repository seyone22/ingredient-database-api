  ...
  const searchIngredients = async (query: string) => {
    ...
    try {
      const res = await fetch(
        `/api/ingredients?query=${encodeURIComponent(query)}&autosuggest=true&limit=10`,
      );
      if (!res.ok) return;
      const data = await res.json();
      ...
      <main className="flex-1 w-full max-w-3xl mx-auto px-4 py-12 flex flex-col gap-8">
        ...
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">
            Link raw supermarket products to canonical database ingredients.
          </p>
        </div>
          ...
          <Card className="bg-muted/50 border-dashed text-center py-16">
            <CardContent className="flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-muted-foreground max-w-md">
                There are currently no unmapped products in the database.
                Excellent work keeping the catalog clean.
              </p>
              <Button
                variant="outline"
