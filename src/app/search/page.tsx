...
import {IngredientSearchResponse, searchIngredients} from "@/services/ingredientService";
                ...
                <ul className={styles.resultsList}>
                    {results.map((ing) => (
                        <li key={ing.name} className={styles.card}>
                            ...
                            <p><strong>Provenance:</strong> {ing.provenance || "Unknown"}</p>
                            {ing.flavor_profile && <p><strong>Flavor:</strong> {ing.flavor_profile.join(", ")}</p>}
                            {ing.comment && <p dangerouslySetInnerHTML={{__html: ing.comment}}></p>}
                        </li>
                    ))}
                </ul>
