...
function productsWithSource() {
  return db
    .select({
      ...productColumns,
      source: { id: priceSources.id, name: priceSources.name },
    })
    .from(products)
...
export async function fetchAllProducts({
  page = 1,
  limit = 50,
}: FetchAllOptions = {}): Promise<ProductListResponse> {
  try {
    const offset = (page - 1) * limit;
  ...
  for (const fetcher of FETCHERS) {
    ...
    for (const raw of rawItems) {
      ...
      const existing = await db
        ...
        .where(
          and(
            eq(products.externalId, mapped.externalId),
            eq(products.sourceId, mapped.sourceId),
          ),
        )
        .limit(1);
      ...
      const stockChanged =
        previous &&
        mapped.stockInHand != null &&
        previous.stockInHand !== mapped.stockInHand;
