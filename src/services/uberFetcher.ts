...
export class UberEatsKeellsFetcher extends SupermarketFetcher {
  ...
  private STORE_URL =
    "https://www.ubereats.com/store/keells-groceries-union-place/82Lx1HEQXIef2bjoQqrPrg?diningMode=DELIVERY&pl=%3D%3D";
  ...
  private async ensureSession() {
    ...
    const headersPromise = new Promise<Record<string, string>>((resolve) => {
      page.on("request", (request) => {
        if (
          request.url().includes("getCatalogPresentationV2") &&
          request.method() === "POST"
        ) {
    ...
    try {
      ...
      console.log(
        "-> Base HTML loaded. Waiting for the internal API calls to fire...",
      );
        ...
        new Promise<Record<string, string>>((_, reject) =>
          setTimeout(
            () =>
              reject(
                new Error(
                  "Timeout waiting for getCatalogPresentationV2 to fire in the browser",
                ),
              ),
            15000,
          ),
  ...
  async fetchFromSource(params: UberEatsFetchParams): Promise<any[]> {
    ...
    if (!this.extractedHeaders)
      throw new Error("Failed to acquire session headers");
    ...
    if (!params.storeUuid || !params.sectionUuid) {
      throw new Error(
        "UberEats scraper requires 'storeUuid' and 'sectionUuid' in parameters.",
      );
    }
    ...
    console.log(
      `-> Starting data extraction for section: ${params.sectionUuid}`,
    );
    ...
    do {
      ...
      if (!response.ok) {
        throw new Error(
          `UberEats fetch failed: ${response.status} ${response.statusText}`,
        );
      }
      ...
      for (const section of catalogs) {
        const items =
          section?.payload?.standardItemsPayload?.catalogItems || [];
      ...
      if (itemsFoundInPage === 0 && hasMore) {
        console.warn(
          "-> Warning: Uber claimed 'hasMore' but returned 0 items. Breaking loop to prevent infinite hang.",
        );
        break;
      }
    } while (hasMore);
  ...
  mapToProduct(raw: any, ingredientId?: string): typeof products.$inferInsert {
    if (!this.sourceId) {
      throw new Error(
        "UberEatsKeellsFetcher: sourceId not resolved — call fetchFromSource() first",
      );
    }
